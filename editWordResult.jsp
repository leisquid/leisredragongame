...
<%
    ...
    if(login.getIsLoggedIn() == false) {
        ...
        try {
            ...
            originalReadingId = Integer.parseInt(request.getParameter("originalReadingId"));
        ...
        if((isExceptionOccurs == false) && (info.getReadingId() > 0)) {
            try {
                querySql =
                        "UPDATE " + tableName + "\n" +
                        "SET\n" +
                        "reading_id = " + info.getReadingId() + ",\n" +
                        "panel_id = " + info.getPanelId() + ",\n" +
                        "reading = \'" + info.getReading() + "\',\n" +
                        "read_type = " + info.getReadType() + ",\n" +
                        "detail_text = \'" + info.getDetailText() + "\',\n" +
                        "head_symbol = \'" + info.getHeadSymbol() + "\',\n" +
                        "tail_symbol = \'" + info.getTailSymbol() + "\',\n" +
                        "is_character = " + info.getIsCharacter() + "\n" +
                        "WHERE reading_id = " + originalReadingId + ";\n";
                preparedStatement = connection.prepareStatement(querySql);
                int returnedValue = preparedStatement.executeUpdate();
                if (returnedValue != 0) {
                    backInfo = info.getReadingId() + "：“" + info.getReading() + "”更新成功！<br />" +
                            "可以<a href=\"showInfo.jsp?infoMode=1&search=" + info.getReadingId() + "\">" +
                            "点我查看修改后词条的详情</a>。";
                } else {
                    backInfo = "词条更新失败！SQL 返回值：" + returnedValue;
                }
            } catch (SQLException exception) {
                isExceptionOccurs = true;
                exceptionInfo = exceptionInfo + "该词条 ID 可能与其他词条 ID 雷同，修改失败！<br />";
            }
        }
