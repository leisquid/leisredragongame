...
<%
    ...
    if(isExceptionOccurs != true) {
        String querySqlForAllWordCount = "select count(*) from " + tableName +
                                        " where read_type = 1 or read_type = 2;";
        try {
            ...
            if(resultSet.next()) {
                ...
                isExceptionOccurs = true;
                exceptionInfo = exceptionInfo + "没有查到所有普通和隐藏词条的总数！<br />";
            }
        } catch (SQLException exception) {
            isExceptionOccurs = true;
            exceptionInfo = exceptionInfo + "无法查询普通和隐藏词条的总数！<br />" + exception.toString() + "<br />";
        }

        // 查询匹配词条。
        String querySqlForAllWord = "select reading_id, reading, read_type, panel_id, tail_symbol from "
                + tableName + " where read_type = 1 or read_type = 2;";
        try {
            ...
            isExceptionOccurs = true;
            exceptionInfo = exceptionInfo + "查询普通和隐藏词条出错了！<br />" + exception.toString() + "<br />";
        }
    }
            ...
            <table width="1200" class="lookupTable">
                ...
                <tr>
                    <td colspan="10">
                        ...
                        <h2>请点击嘉夜选出的第一张牌</h2>
                        由于嘉夜“头脑简单，只会打架”，因此她第一张牌不会选 <span class="pcrColor">PCR 读法的词条</span>。或者<a href="gameStartWithAllWords.jsp">点我从所有词条开始</a>，还是<a href="listAll.jsp">先点我栞栞全图鉴</a>？<br />
                        您也可以<a href="sortSettings.jsp">点我修改游戏模式时词条的排序方法</a>。
                        <br />
                        <%
                            }
                        %>
