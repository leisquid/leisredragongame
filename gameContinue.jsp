...
<%
    ...
    // 要从 url 中获得要接的韵母。形式：gameContinue.jsp?symbol=ang
    String symbol = new String();
    try {
        symbol = request.getParameter("symbol");
    } catch (Exception exception) {
        isExceptionOccurs = true;
        exceptionInfo = "没有指定韵母！<br />" + exception.toString() + "<br />";
    }
    ...
    if(isExceptionOccurs != true) {
        ...
        try {
            ...
            if(resultSet.next()) {
                ...
                isExceptionOccurs = true;
                exceptionInfo = exceptionInfo + "没有查到所有韵母匹配的词条的总数！<br />";
            }
        } catch (SQLException exception) {
            isExceptionOccurs = true;
            exceptionInfo = exceptionInfo + "无法查询所有韵母匹配的词条的总数！<br />" + exception.toString() + "<br />";
        }

        // 查询匹配词条。
        String querySqlForAllWord = "select reading_id, reading, read_type, panel_id, tail_symbol from "
                + tableName + " where head_symbol like \'" + symbol + "\'" + sorting.generateSortingSql() + ";";
        try {
